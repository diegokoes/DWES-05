...
//        }
    ...
    public ResponseEntity<ProductoDTO> findBySku(@PathVariable String sku) {
        ...
        return ResponseEntity.ok(productoService
                .convert2ProductoDTO(productoRepository.findProductoBySku(sku).orElse(null)).get());

    }

    @PutMapping("/update-sku/{sku}")
    public ResponseEntity<ProductoDTO> updateSku(@PathVariable String sku, @RequestBody ProductoDTO detalles) {
        return productoRepository.findProductoBySku(sku)
                .map(producto -> {
                    producto.setNombre(detalles.getNombre());
                    producto.setPrecio(detalles.getPrecio());
                    productoRepository.save(producto);
                    return ResponseEntity.ok(productoService.convert2ProductoDTO(producto).get());
                }).orElse(ResponseEntity.notFound().build()); //si el optional está vacío devuelve un 404
    }
